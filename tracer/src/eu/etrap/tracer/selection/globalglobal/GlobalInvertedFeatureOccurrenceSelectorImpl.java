 ...
 */
public class GlobalInvertedFeatureOccurrenceSelectorImpl extends AbstractGlobalSelection implements Selection {

    @Override
    public void init() throws ConfigurationException {
        super.init();
        strSortedOutFile = FileManager.getSelectionSortedIDFFileName();
        strTaxonomyCode = "01-01-01-03-01";
    }

    @Override
    protected void doWeightFeatures() throws SelectionException {
        try {
            String strEntropyFileName = FileManager.getSelectionIDFFileName();
            ...
            while (objIter.hasNext()) {
                ...
                if (intFeatID > 0) {
                    ...
                    double p = (double) intFeatFreq / (double) this.objFeatureDistribution.get(Constants.NUMBER_OF_REUSE_UNITS);
                    double idf = -1 * Math.log(p) / Math.log(2);
                    int intScaledFactor = (int) (idf * (double) intScaleFactor);
    ...
    protected void buildByteArray() throws SelectionException {
        ...
        try {
            BufferedReader objReader = new BufferedReader(new FileReader(FileManager.getSelectionSortedIDFFileName()));
            String strLine = null;
